...
def build_report_tex(tunnel: Dict[str, Any],
    ...
    register_rows = "".join(
        ...
        for d in sorted(defects,
                        key=lambda x: {"HIGH": 0, "MEDIUM": 1}.get(
                            x.get("priority"), 2))
    )
    ...
    case_files = (
        "\n".join(_defect_case_file(d) for d in sorted(
            defects, key=lambda x: {"HIGH": 0, "MEDIUM": 1}.get(
                x.get("priority"), 2)))
        if include_case_files else
        "\\emph{Per-defect case files were excluded from this report.}"
    )
...
Coverage = rings with evidence from \\(\\ge\\)3 of 4 sensing sources.
Low-coverage sections are candidates for follow-up survey.

\\medskip
\\begin{{tabular}}{{@{{}}lll@{{}}}}\\toprule
Section & Chainage range & Coverage \\\\ \\midrule
{coverage_rows}\\bottomrule
\\end{{tabular}}
